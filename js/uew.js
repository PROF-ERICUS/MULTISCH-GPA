...
function classifyGPA(gpa) {
  if (gpa >= 3.50) return 'First Class';
  if (gpa >= 3.00) return 'Second Class Upper Division';
  if (gpa >= 2.50) return 'Second Class Lower Division';
...
function calculateSemesterGPA(button) {
  ...
  courseRows.forEach(row => {
    ...
    // Check if Course Name is filled, otherwise it's incomplete
    const courseName = inputs[0].value.trim();

    if (courseName === '' || isNaN(credit) || isNaN(marks)) {
      incompleteCourses++;
      // Skip calculation for incomplete rows, but still allow grade display for typed marks
  ...
  if (semesterCredits > 0) {
    ...
    for (let i = 0; i <= Array.from(allSections).indexOf(semesterSection); i++) {
      ...
      rows.forEach(row => {
        ...
        const credit = parseFloat(inputs[1].value);
        const marks = parseFloat(inputs[2].value);
        if (isNaN(credit) || isNaN(marks) || inputs[0].value.trim() === '') return; // Skip incomplete
    ...
    if (incompleteCourses > 0) {
      // Message for user, but don't disrupt flow
    }
  } else {
...
function updateMainCGPA() {
  ...
  if (totalCredits > 0) {
    ...
    document.getElementById('cgpaResult').innerText = 'Enter course details to calculate overall CGPA.';
  }
}
...
function resetForm() {
  ...
  addSemester(); // Start with a fresh semester and save the new state
}
...
window.onload = function () {
  // Assuming the user authentication script is still in the HTML file's <script> block
...
function printResults() {
  let printContent = `
    <div style="text-align:center;">
      <img src="uew.logo.png" width="100" />
      <h2>University of Education-Winneba - GPA Report</h2>
    </div>
  `;
